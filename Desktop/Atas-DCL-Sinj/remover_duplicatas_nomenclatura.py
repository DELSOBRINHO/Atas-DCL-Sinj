...
print("\n" + "="*70)
print("REMOVER DUPLICATAS POR NOMENCLATURA")
print("="*70)

print(f"\nTotal de atas antes: {len(atas)}")

# Remover duplicatas por nomenclatura (mantém a primeira ocorrência)
vistas = set()
atas_unicas = []
...
for ata in atas:
    ...
    if nomenclatura not in vistas:
        ...
        print(f"Duplicata removida: {ata['sessao_num']}ª ({ata['data_real']}) - {nomenclatura}")
...
print(f"Duplicatas removidas: {len(atas) - len(atas_sorted)}")
